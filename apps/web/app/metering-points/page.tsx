...
export default async function MeteringPointsPage() {
  ...
  return (
    <div className="min-h-screen p-8 pb-20 sm:p-20 bg-slate-50 text-slate-900 font-[family-name:var(--font-geist-sans)] flex justify-center">
      <main className="flex flex-col gap-8 w-full max-w-2xl">
        ...
        {error === 'failed' && (
          <div className="p-4 border border-red-800 bg-red-900/20 text-red-400 rounded-lg">
        ...
        <div className="flex flex-col gap-4">
          {meteringPoints && meteringPoints.length > 0 ? (
                      ...
                      <div className="flex flex-col gap-2 w-full">
                        {point.invitations.map((inv: any) => (
                          <div key={inv.id} className="flex flex-col items-start sm:items-end border border-teal-200 bg-teal-50 p-3 rounded-xl gap-1 shadow-sm w-full">
                            ...
                            <Link href="/metering-point-invitations" className="text-xs mt-1 text-teal-600 hover:text-teal-800 font-bold flex items-center gap-1">
            ...
            <div className="text-slate-500 text-sm italic">Zatiaľ ste nezaregistrovali žiadne pripojené miesta.</div>
          )}
        </div>
      </main>
    </div>
